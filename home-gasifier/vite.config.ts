...
export default defineConfig(({ mode, isSsrBuild }) => ({
  ...
  ssr: {
    // Always bundle react-router so Rollup/esbuild handles ESM interop
    // rather than externalizing it to Node require(), which returns a CJS
    // namespace that Vite's SSR module runner cannot resolve named exports from.
    noExternal: isSsrBuild ? true : ["react-router", "react-router"]
  },
